...
def PDF(dict):
    try:
        ...
        if m:
            ...
            html = template.render(context)
            print type(html)
            # data = open('E:\BaiduNetdiskDownload\MobSF\Mobile-Security-Framework-MobSF-0.9.2\templates').read()
            result = open('C:\MobSF\Mobile-Security-Framework-MobSF-0.9.2\uploads/'+MD5+'/'+MD5+'.pdf', 'w')
            # pdfkit.from_string(html, result)
...
def StaticAnalyzer(request):
    for parent, dirnames, filenames in os.walk('E:/APK'):
        for F in filenames:
            if F.endswith('.apk'):
                file= open('E:/APK/' + F, 'rb')
                try:
                    # Input validation
                    dict = {'type': file.name[-3:],
                            'name': file.name[7:],
                            'checksum': md(file)
                            }
                    TYP = dict['type']
                    m = re.match('[0-9a-f]{32}', dict['checksum'])
                    if ((m) and (dict['name'].lower().endswith('.apk') or dict['name'].lower().endswith('.zip')) and (
                        (TYP == 'zip') or (TYP == 'apk'))):
                        DIR=settings.BASE_DIR   # BASE DIR
                        APP_NAME = dict['name']  # APP ORGINAL NAME
                        MD5 = dict['checksum']  # MD5
                        APP_DIR = os.path.join(settings.UPLD_DIR, MD5 + '/')  # APP DIRECTORY
                        TOOLS_DIR = os.path.join(DIR, 'StaticAnalyzer/tools/')  # TOOLS DIR
                        # DWD_DIR = settings.DWD_DIR
                        print "[INFO] Starting Analysis on : " + APP_NAME
                        RESCAN = str(dict.get('rescan', 0))
                        if TYP == 'apk':
                            # Check if in DB
                            DB = StaticAnalyzerAndroid.objects.filter(MD5=MD5)
                            if DB.exists() and RESCAN=='0':
                                ...
                                API, DANG, URLS, DOMAINS, EMAILS, CRYPTO, OBFUS, REFLECT, DYNAMIC, NATIVE = CodeAnalysis(APP_DIR,
                                                                                                                         MD5,
                                                                                                                         PERMISSIONS,
                                                                                                                         "apk")
                                print "\n[INFO] Generating Java and Smali Downloads"
                            ...
                            if DB.exists() and RESCAN == '0':
                                ...
                                if Valid and pro_type == 'ios':
                                    ...
                                    return HttpResponseRedirect(
                                        '/StaticAnalyzer_iOS/?name=' + APP_NAME + '&type=ios&checksum=' + MD5)
                                CERTZ = GetHardcodedCertKeystore(FILES)
                                print "[INFO] ZIP Type - " + pro_type
                                if Valid and (pro_type == 'eclipse' or pro_type == 'studio'):
                                    # ANALYSIS BEGINS
...
def CertInfo(APP_DIR,TOOLS_DIR):
    try:
        ...
        dat=''
        issued='good'
        # a = '[\n[\n  Version: V3\n  Subject: CN=\xe5\xe2\xe5\xcb\xcd\xf8, OU=mei, O=\xe5\xe2\xe5\xcb\xcd\xf8, L=\xb9\xe3\xd6\xdd\xca\xd0, ST=\xb9\xe3\xb6\xab\xca\xa1, C=CN\n  Signature Algorithm: SHA1withRSA, OID = 1.2.840.113549.1.1.5\n\n  Key:  \n  Validity: [From: Wed Jan 09 17:33:37 CST 2013,\n               To: Fri Dec 16 17:33:37 CST 2112]\n  Issuer: CN=\xe5\xe2\xe5\xcb\xcd\xf8, OU=mei, O=\xe5\xe2\xe5\xcb\xcd\xf8, L=\xb9\xe3\xd6\xdd\xca\xd0, ST=\xb9\xe3\xb6\xab\xca\xa1, C=CN\n  SerialNumber: [    50ed3971]\n\n]\n  Algorithm: [SHA1withRSA]\n  Signature:\n0000: 90 6F CB C1 39 21 19 A9   5E 62 20 22 5F DB 4F 15  .o..9!..^b "_.O.\r\n0010: 47 F5 C4 91 4B 3A 2C A4   A3 76 08 06 19 B7 7E 15  G...K:,..v......\r\n0020: A0 E3 F1 2B 8E 3F 46 43   32 D7 DA 9E 07 03 46 46  ...+.?FC2.....FF\r\n0030: 48 D3 96 AA C3 35 82 83   46 C1 20 BA 5D BA C0 53  H....5..F. .]..S\r\n0040: 4C 32 D2 19 D6 DB 5E 15   BC DA 89 3F 4F 07 83 36  L2....^....?O..6\r\n0050: 4D 91 0D F6 56 DA 26 94   C4 B4 06 39 76 C3 D0 99  M...V.&....9v...\r\n0060: 51 6B 41 BF EF F8 73 7C   48 9C 2B 28 C3 1E 52 FC  QkA...s.H.+(..R.\r\n0070: F4 EA 31 86 3E BC F2 E7   B9 43 A6 B6 AB 49 BD 8A  ..1.>....C...I..\r\n\n]\r\n'
        print type(subprocess.check_output(args))
